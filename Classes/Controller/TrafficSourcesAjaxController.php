...
final readonly class TrafficSourcesAjaxController
{
     ...
     * @return array{icon: string, title: string, showSiteSelect: bool, items: list<array{label: string, value: string, tone: string, icon: string, change: string|null, changeTone: string}>}
     */
    private function buildSection(string $icon, string $title, array $items): array
    {
        return ['icon' => $icon, 'title' => $title, 'showSiteSelect' => false, 'items' => $items];
    }
