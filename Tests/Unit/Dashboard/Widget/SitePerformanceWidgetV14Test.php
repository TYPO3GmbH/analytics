...
final class SitePerformanceWidgetV14Test extends UnitTestCase
{
    protected bool $resetSingletonInstances = true;

    private WidgetConfigurationInterface&MockObject $configuration;
    private SitePerformanceServiceInterface&MockObject $sitePerformanceService;
    ...
    protected function setUp(): void
    {
        ...
        $this->configuration->method('getIdentifier')->willReturn('analyticsSitePerformanceV14');

        $this->sitePerformanceService = $this->createMock(SitePerformanceServiceInterface::class);
        $this->sitePerformanceService->method('loadPerformanceData')->willReturn(null);
        ...
        $this->view->method('render')->willReturn('<div>site-performance-html</div>');

        $this->viewFactory = $this->createMock(ViewFactoryInterface::class);
        $this->viewFactory->method('create')->willReturn($this->view);
    }

    private function makeWidget(): SitePerformanceWidgetV14
    {
        return new SitePerformanceWidgetV14(
            $this->configuration,
            $this->sitePerformanceService,
            $this->siteProvider,
            $this->uriBuilder,
            $this->viewFactory,
        );
    ...
    public function renderWidgetLabelIncludesMetaWhenShowMetaIsTrue(): void
    {
        ...
        self::assertSame('dashboardWidget.sitePerformance.title (Demo Site · 7 days)', $result->label);
    }
    ...
    public function renderWidgetLabelExcludesMetaWhenShowMetaIsFalse(): void
    {
        ...
        self::assertSame('dashboardWidget.sitePerformance.title', $result->label);
    }
